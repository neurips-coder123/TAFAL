...
def train_negation_control_target(args, target_task):
    ...
    control_task = "mnli"
    control_loader = get_glue_calib_loader(
        control_task,
        tokenizer,
        nsamples=args.nsamples,
        seqlen=base_model.config.max_position_embeddings,
    ...
    min_steps = min(len(control_loader), len(target_loader))

    # -------------------------
    # Control classifier (frozen)
    # -------------------------
    control_classifier = utils.load_classifier(
        eval.model_path_template.format(name=control_task)
    ).classifier.to(DEVICE).eval()

    for p in control_classifier.parameters():
        p.requires_grad_(False)
    ...
    for epoch in range(args.epochs):
        ...
        control_iter = iter(control_loader)
        ...
        for _ in range(min_steps):
            ...
            try:
                batch_c = next(control_iter)
            except StopIteration:
                control_iter = iter(control_loader)
                batch_c = next(control_iter)

            feats_c = model(
                batch_c["input_ids"].to(DEVICE),
                batch_c["attention_mask"].to(DEVICE)
            )

            logits_c = control_classifier(feats_c)
            loss = loss + criterion(
                logits_c,
                batch_c["labels"].to(DEVICE),
            )
            ...
            loss.backward()
            torch.nn.utils.clip_grad_norm_([model.coef], 1.0)
            optimizer.step()
...
def run_negation_inference(args, task):
    ...
    # -------------------------
    # Only load needed classifiers: task + mnli
    # -------------------------
    needed_tasks = [task, "mnli"]
    ...
    # -------------------------
    # Load + filter data: task + mnli
    # -------------------------
    data = utils.from_json(args.data_path)

    task_id = eval.glue_data_id_map[task]
    mnli_id = eval.glue_data_id_map["mnli"]
    valid_ids = [task_id, mnli_id]

    data = [item for item in data if item["dataset_ids"] in valid_ids]


    coef = torch.load(
        os.path.join(args.outdir, f"negation_{task}.pt")
    ).to(DEVICE)

    task_vector = build_task_vector(
        base_param,
        finetuned_params["mnli"],
        param_names,
    )

    eval_pred = defaultdict(lambda: defaultdict(list))

    for item in tqdm.tqdm(data, desc=f"infer target={task}"):

        dataset = list(eval.glue_data_id_map.keys())[item["dataset_ids"]]

        merged = {
            name: base_param[name] - coef[i] * task_vector[i]
        ...
        ans = eval.compute_single_metrics(
            utils.SimpleNamespace(
                predictions=np.concatenate(
                    eval_pred[dataset]["predictions"]
                ),
                label_ids=np.array(
                    eval_pred[dataset]["label_ids"]
                ),
            ),
            dataset,
        )["averaged_scores"]
    ...
    utils.save_excel(
        metrics,
        os.path.join(
            args.outdir, f"negation_target_{task}.xlsx"
        ),
    )

    print(f"\n>>> RESULTS FOR TARGET {task}")
...
def main(
    *,
    ...
    for task in models_name:
        ...
        run_negation_inference(args, task)
