...
from model import MaskedRobertaForSequenceClassification, materialize_masks
...
# model_path_template='./local_roberta_models/textattack_roberta-base-{name}'
# head_path_template='./local_roberta_models/textattack_roberta-base-{name}/classifier_head.pt'
model_path_template='./roberta/{name}/roberta-base_lr1e-05'
# model_path_template='./linearized_logits/{name}'
# model_path_template='./talos_roberta/{name}'
# model_path_template='./linearlised/{name}'
# model_path_template='./linearized_cls/{name}'
# model_path_template='./results_{name}_noreg/best_model'
head_path_template='./roberta/{name}/roberta-base_lr1e-05/classifier_head.pt'
# head_path_template='./linearized_logits/{name}/classifier_head.pt'
...
def run_eval_glue(
    *, 
    ...
    if args.maskllm:
        print(f"################################{args.model}###################################")
        model = MaskedRobertaForSequenceClassification.from_pretrained(args.model).to('cuda')
    else:
        model = AutoModelForSequenceClassification.from_pretrained(args.model).to('cuda')
    
    model.eval()
    tokenizer = AutoTokenizer.from_pretrained(args.model)
    model = materialize_masks(model)
    for name, module in model.named_modules():
        if isinstance(module, nn.Linear):
            ...
            for p in module.parameters():
                total += p.numel()
                zeros += (p == 0).sum().item()
            print(f"Name: {name}, Total: {total}, Zeros: {zeros}")
