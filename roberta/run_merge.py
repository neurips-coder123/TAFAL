...
def run_pretrained(
    ...
    for data_item in (data):
        ...
        if data_name in args.base_model:
...
def run_merge(
    ...
    if args.merge_method == 'hessian':
        models_to_merge = [
            (models_finetuned[name].to(DEVICE), torch.load(f'./outs/hessian/256/{name}.pt'))
            for name in args.src_merge
        ]
        args.models_to_merge = [(param(m),n) for m,n in models_to_merge]
        for model,_ in args.models_to_merge:
            model.filter(filter_func)
    elif args.merge_method == 'hessian_neg':
        print("Hessian_Neg")
        models_to_merge = [
            (models_finetuned[name].to(DEVICE), torch.load(f'./outs/hessian/512/{name}.pt'))
            for name in args.src_merge
        ]
        args.models_to_merge = [(param(m),n) for m,n in models_to_merge]
        for model,_ in args.models_to_merge:
            model.filter(filter_func)
        args.neg_task_index = [i for i,x in enumerate(args.src_merge) if x in args.negation]
    elif args.merge_method == 'task_arithmetic_neg':
        print("TA_Neg")
        models_to_merge = [
    ...
    if args.data_path is not None:
        ...
        if 'neg' in args.merge_method:
            for i in args.negation:
                metrics['_src_merge'] = metrics['_src_merge'].replace(f"+{i}", f"-{i}")
        ...
        data_items = ["cola", "mrpc", "mnli", "qnli", "rte", "sst2", "stsb", "qqp"]
        for item in data_items:
            if item not in models_finetuned:
                models_finetuned[item] = utils.load_classifier(eval.model_path_template.format(name=item)).to(DEVICE)
