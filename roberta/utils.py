...
import torch
import transformers
from model import MaskedRobertaForSequenceClassification, materialize_masks, fuse_masks_inplace, save_all_param_masks
...
def load_classifier(model_name: str, maskllm=False, save_masks=False, dtype=torch.float32, save_classifier_head=True):
    if maskllm:
        model = MaskedRobertaForSequenceClassification.from_pretrained(
              model_name, torch_dtype=dtype, device_map="cpu",
        )
        if save_masks:
            save_all_param_masks(model,f"{model_name}/masks.pt")
        #     model = fuse_masks_inplace(model,
        #         AutoModelForSequenceClassification.from_pretrained(
        #             finetuned, torch_dtype=dtype, device_map="cpu",
        #         ))
        # else:
...
def load_lm(model_name: str, dtype=torch.float32):
