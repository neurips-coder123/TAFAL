...
OUTDIR = "./outs/final_layer_analysis"
...
BASE_MODEL = "./outs/task_arithmetic"


for task in TASKS:
    ATLAS_WEIGHT_PATH = "./outs/atlas/negation_mnli.pt"
    ...
    m = utils.load_classifier(
        eval.model_path_template.format(name=task)
    ).eval()
    p = param(m)
    p.filter(lambda n, _: is_encoder_param(n))
    finetuned_params = p
    del m
    gc.collect()

    task_vectors = [
        finetuned_params[n] - base_param[n]
        for n in param_names
    ]

    params = {}
    atlas_coef = torch.load(ATLAS_WEIGHT_PATH, map_location="cpu")
    for i, name in enumerate(param_names):
        # delta =  atlas_coef[i] * task_vectors['mnli'][i]
        params[name] = base_param[name].to(DEVICE) + atlas_coef[i]*task_vectors[i].to(DEVICE)
